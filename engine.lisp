...
(defun quit (&optional code)
  ...
  #+lucid (lcl:quit code)
  #+sbcl (sb-ext:quit
          :unix-status (typecase code (number code) (null 0) (t 1)))
...
(defparameter *search-fn* (alexandria:curry #'alpha-beta most-positive-fixnum most-negative-fixnum (get-option :depth-limit) #'evaluate))
(defparameter *shallow-search-fn* (alexandria:curry #'alpha-beta most-positive-fixnum most-negative-fixnum 2 #'evaluate))
...
(defun process-cecp-command (cmd)
  ...
  (cond ((ppcre:register-groups-bind ((#'parse-integer arg))
        ...
        ((ppcre:scan "^level\\s\\d+\\s[0-9:]+\\s\\d+$" cmd) (unsupported-command cmd))
        ((ppcre:scan "^st\\s\\d+$" cmd) (unsupported-command cmd))
        ((ppcre:register-groups-bind ((#'parse-integer arg))
             ("^sd\\s(\\d+)$"cmd)
           (set-option! :depth-limit arg)))
        ((ppcre:scan "^nps\\s\\d+$" cmd) (unsupported-command cmd))
        ...
        ((ppcre:scan "^memory\\s\\d+$" cmd) (unsupported-command cmd))
