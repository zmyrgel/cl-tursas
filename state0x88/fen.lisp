...
(defun make-state (fen)
  ...
  (destructuring-bind (board turn cast en-pass half full)
    ...
    (let* ((game-board (fen-board->0x88board board))
           (state (make-State0x88 :board game-board)))
      (add-pieces state)
      ...
      (let ((in-check (if (string= turn "w")
                          (threatenedp game-board (king-index game-board +white+) +black+)
                          (threatenedp game-board (king-index game-board +black+) +white+))))
        (fill-square! game-board +check-store+ (if in-check 1 0))
