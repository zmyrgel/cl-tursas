...
(deftest white-promotion-tests
  (testing "white promotion moves"
...
(deftest board-tests
  (testing "board stuff"
    (let ((board (tursas.state0x88::State0x88-board (tursas.state0x88::fen->state "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"))))
      ...
      (ok (= (tursas.state0x88::opponent tursas.state0x88::+white+) tursas.state0x88::+black+)
          "white players opponent should be black")
      (ok (= (tursas.state0x88::opponent tursas.state0x88::+black+) tursas.state0x88::+white+)
          "black players opponent should be white")

      ;;(is (promotionp piece move) )

      (ok (eql (tursas.state0x88::piece-name tursas.state0x88::+white-king+) #\K)
          "white kings piece name should be K")
      (ok (eql (tursas.state0x88::piece-name tursas.state0x88::+black-queen+) #\q)
          "black queens piece name should be q")
      (ok (eql (tursas.state0x88::piece-name tursas.state0x88::+white-pawn+) #\P)
          "white pawns piece name should be P")
      (ok (eql (tursas.state0x88::piece-name tursas.state0x88::+black-king+) #\k)
          "black kings piece name should be k")

      (ok (= (tursas.state0x88::piece-value #\K) tursas.state0x88::+white-king+)
          "piece value of K should be equal to white kings value")
      (ok (= (tursas.state0x88::piece-value #\q) tursas.state0x88::+black-queen+)
          "piece value of Q should be equal to black queens value")
      (ok (= (tursas.state0x88::piece-value #\P) tursas.state0x88::+white-pawn+)
          "piece value of P should be equal to white pawn")
      (ok (= (tursas.state0x88::piece-value #\k) tursas.state0x88::+black-king+)
      ...
      (ng (tursas.state0x88::board-index-p #x78)
          "0x78 should not be board index, outside board indexes but within the array")
      (ng (tursas.state0x88::board-index-p #x128)
      ...
      (ng (tursas.state0x88::empty-square-p board #x00)
      ...
      (ok (= (tursas.state0x88::rank #x10) 1)
          "index 0x10 should be on second rank")
      (ok (= (tursas.state0x88::rank #x33) 3)
          "index 0x22 should be on rank 4")
      (ok (= (tursas.state0x88::rank #x42) 4)
          "index 0x42 should be on rank 2")
      (ok (= (tursas.state0x88::rank #x46) 4)
          "index 0x46 should be on rank 7")

      (ok (= (tursas.state0x88::file #x10) 0)
          "0x10 should be on first file")
      (ok (= (tursas.state0x88::file #x34) 4)
          "0x33 should be on fourth file")
      (ok (= (tursas.state0x88::file #x57) 7)
      ...
      (ng (tursas.state0x88::same-file-p #x20 #x34)
          "indexes 0x20 and 0x34 should on different file")

      (ng (tursas.state0x88::same-rank-p #x10 #x50)
          "index 0x10 and 0x50 should not be on same rank")
      (ng (tursas.state0x88::same-rank-p #x10 #x53)
      ...
      (ok (= (tursas.state0x88::square-color #x10) tursas.state0x88::+white+)
          "square color of index 0x10 should be white")
      (ok (= (tursas.state0x88::square-color #x11) tursas.state0x88::+black+)
      ...
      (ok (= (tursas.state0x88::board-ref board #x10) tursas.state0x88::+white-pawn+)
          "index 0x10 should contain white pawn in start position")
      (ok (= (tursas.state0x88::board-ref board #x30) tursas.state0x88::+empty-square+)
          "index 0x30 should be empty on start position")

      (ng (tursas.state0x88::fill-square! board #x45 20)
          "filling square 0x45 should not succeed... for reasons I can't think of currently")

      (ok (eql (tursas.state0x88::king-index board tursas.state0x88::+white+) #x04)
